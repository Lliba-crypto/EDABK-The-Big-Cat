...
// Thêm 2 biến này vào ngay phía trên hàm HAL_SYSTICK_Callback
volatile uint8_t flag_read_imu = 0;
static uint32_t imu_timer_count = 0;

// Ghi đè hàm Callback mới
void HAL_SYSTICK_Callback(void) {
    // Hàm này được gọi tự động mỗi 1ms bởi SysTick_Handler
    imu_timer_count++;

    // Cứ mỗi 10ms, ta bật cờ báo hiệu đã đến lúc đọc IMU
    if (imu_timer_count >= 10) {
        flag_read_imu = 1;
        imu_timer_count = 0;
    }
}
