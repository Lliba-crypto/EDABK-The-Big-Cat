...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// Bổ sung khai báo extern để lấy biến cờ từ drv8833.c sang
extern volatile uint8_t flag_read_imu;
...
int main(void)
{
  ...
  while (1)
  {
	  // 1. KIỂM TRA IMU MỖI 10ms
	        if (flag_read_imu == 1) {
	            MPU6050_Read_Data(&Raw);
	            // Truyền dt = 0.01s (tương ứng 10ms) để tích phân góc chính xác
	            updateGyroAngle(Raw.Gz, 0.01f);

	            // Sau khi đọc xong thì hạ cờ xuống
	            flag_read_imu = 0;
	        }
